...
// action creators
import {
  confirmSetListing, emptyFilterConfirmSet, filterConfirmSet,
  confirmSetDropDownValue,
} from '../../redux/ConfirmSet/actionCreator';

// filters file
import { ConfirmSetsFilteration } from './ConfirmSetsFilteration';
...
const confirmSetList = ({
  ...
  const [previewedConfirmSetPagination, setPreviewedConfirmSetPagination] = useState({});
  ...
  const [confirmSetFilteration, setConfirmSetFilteration] = useState('CONFIRMING');
  ...
  useEffect(() => {
    ...
    if (hasRole([Roles.ADMIN])) {
      if (companiesDropDown.label !== null) {
      ...
      else if ((dateFilteration.from !== '' && dateFilteration.to !== '') || confirmSetFilteration) {
        dispatch(filterConfirmSet({
          ...
          fromDate: dateFilteration.from !== '' ? dateFilteration.from : undefined,
          toDate: dateFilteration.to !== '' ? dateFilteration.to : undefined,
          confirmSetFilteration }, []));
      }
    }

  }, [companiesDropDown.label, dateFilteration.from, dateFilteration.to, confirmSetFilteration]);

  useEffect(() => {
    if (hasRole([Roles.ADMIN])) {
      const radioButtonsFilteration = confirmSetFilteration && previewedConfirmSet.filter(obj =>
        (obj?.status === confirmSetFilteration)) ? filteredCompaniesDropdown : confirmSets;

      const dropDownDateFilteration = (filteredCompaniesDropdown?.length ||
        (dateFilteration.from !== '' && dateFilteration.to !== ''))
        &&
        radioButtonsFilteration.filter(obj =>
          (obj.company.name === companiesDropDown?.label)
          ||
          (obj.createdAt >= dateFilteration?.from && obj.createdAt <= dateFilteration?.to)
        ) ? filteredCompaniesDropdown : confirmSets;
      ...
      if (scrollLoading) {
        ...
        setpreviewedConfirmSet(radioButtonsFilteration?.length ? [...radioButtonsFilteration] :
          (dropDownDateFilteration?.length && radioButtonsFilteration?.length) ?
            [...dropDownDateFilteration] : previewedConfirmSet);
      } else {
        setpreviewedConfirmSet(radioButtonsFilteration?.length ? [...radioButtonsFilteration] :
          (dropDownDateFilteration?.length && radioButtonsFilteration?.length) ?
  ...
  }, [filteredCompaniesDropdown, confirmSets, companiesDropDown, confirmSetFilteration]);
  ...
  const navigateToConfirmSet = (item) => {
    // history.push(`/confirm-set/${item.id}`);
    history.push({
      pathname: `/confirm-set/${item.id}`,
      search: '?confirm-list',
    });
  ...
  let companiesDropdown, clearSearchText, datePickerFromOnChange, datePickerToOnChange, handleChangeRadioBtnList;
  ...
  if (hasRole([Roles.ADMIN])) {
    ...
    companiesDropdown = (e, action) => {
      ...
      setCompaniesDropDown({ ...companiesDropDown, label: e?.label, value: e?.value, key: e?.key });
    };
  ...
  return (
    <div>
      <Header
        title={translate('confirmSet-list')}
      ...
      <div className="companies-list margin-top-20 margin-bottom-20">
        <div className='row margin-0'>
            ...
            hasRole([Roles.ADMIN]) ?
              <div className='col-4 search-container left-container-filteration padding-top-20 filteration-datePickers'>
                <ConfirmSetsFilteration
                  ...
                  value={companiesDropDown?.label === null ? translate('Company') : companiesDropDown}
          ...
          {/* right container */}
          <div className={`result-container ${leftContainer?.classList?.contains('left-container-filteration') ?
            'col-8' : 'col-12 padding-right-20-important'}`}>
            {
              (companiesDropDown.label || (dateFilteration.from || dateFilteration.to)) &&
              <ViewFilters
            ...
            <InfiniteScrollComponent
              ...
              previewed={[...previewedConfirmSet.map(item => ({
                header: item?.company?.name,
                body: `${formatDate('dd.mm.yyyy', item?.createdAt)} ${item?.status && translate(item?.status)}`,
                realItem: item,
              }))]}
              ...
              hasMore={
                hasRole([Roles.ADMIN]) ?
                  previewedConfirmSetPagination?.totalPages > previewedConfirmSetPagination?.currentPage
                  :
                  confirmSetPagination?.totalPages > confirmSetPagination?.currentPage
              }
...
const mapStateToProps = (state) => ({
  ...
  filteredCompaniesDropdownPagination: state?.confirmSets?.confirmSets_filtered_list_pagination,
...
export const ConfirmSetList = connect(mapStateToProps, mapDispatchToProps)(withTranslation()(confirmSetList));
